 ...
 */
export async function displayVideoInput(video, deviceId) {
    validateInstanceOf(video, HTMLVideoElement);
    video.srcObject = await getVideoInputDevice(deviceId);
}
 ...
 */
export async function populateVideoInputSelect(videoSelect) {
    ...
    for (const device of (await getVideoInputDevices())) {
        if (device.label === "") {
            continue;
        }
 ...
 */
export async function getVideoInputDevice(deviceId) {
